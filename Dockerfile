...
ENTRYPOINT ["/usr/local/bin/mcp-v8", \
  ...
  "--instructions", "Sandboxed V8 JavaScript runtime (run_js) with fetch enabled everywhere and a READ+WRITE scratch directory at /work: persist files with await fs.writeFile('/work/x') and read them back with await fs.readFile('/work/x'); the rest of the filesystem is read-only on /opt/languages. Load language helpers once per run with (0,eval)(await fs.readFile('/opt/languages/bootstrap.js')) -> picat, tlaplus, minizinc, autolisp, lua, jsx, markdown, mermaid. The run_simulation tool boots networked ComputerCraft computers and turtles (rednet, GPS, turtle fake-worlds). A mutable paste store is at https://paste-production.up.railway.app -- write or overwrite a named slot with PUT /<id> (request body = content; POST behaves the same) which echoes the URL, and read it back raw and publicly curl-able at GET /<id> (Cache-Control: no-store). ids match [A-Za-z0-9._-]; DELETE /<id> removes a slot. Because slots are overwritable at a stable URL, this suits hot-reloaded program sources (e.g. a ComputerCraft turtle polling GET /<id>).", \
